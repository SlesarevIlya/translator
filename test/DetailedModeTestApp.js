...
/*.then(function(subscription){
    ...
    .then(function(subscription){
        ...
        var outgoingBatch = getOutgoingBatch(subscription.basic_settings.batch_limit);
        var requests = [];
        console.log("read");
        var rq = SemantriaActiveSession.queueBatchOfDocuments(outgoingBatch, appConfigurationId, true)
            .then(function(res){
                console.log("%d documents queued successfully", outgoingBatch.length);
            });
        requests.push(rq);
...
/*function getOutgoingBatches(batch_limit) {
	var initialTexts = require('./source1.json');
	var batch = [], batches = [];
...
function getOutgoingBatch(batch_limit) {
	console.log("startget");

    //var initialTexts = require('./source.json');
    var fs = require('fs');
    var initialTexts = fs.readFileSync('source1.txt', 'utf8');
    console.log(initialTexts);
	var line = [];
	line.push(initialTexts);

    console.log("startget2");
    var batch = [];
